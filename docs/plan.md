...
| **E — Track A DONE (2026-06-11)** | ✅ Live capture (`PHASE-A-FINDINGS` ⑤/⑧) identified Colab's **ColabMCP** protocol: Colab connects to a local WS as an MCP *server* (subprotocol `mcp`, `?access_token=`) exposing `add_code_cell`/`update_cell`/`run_code_cell`/`get_cells`/`delete_cell`. `BrowserBridgeTransport` **rebuilt** on it (`transport/browser/mcp.py` `McpClient` + bridge): execute via `run_code_cell`, **keep-alive via a no-op cell** (`keepalive=True` — sanctioned, real activity in the authenticated session; the keep-alive the token-auth paths couldn't get), upload/download via cell-exec, `notebook_execution=True`. 14 browser tests (real subprocess-backed fake ColabMCP). Corrects P14's guessed shapes. **Remaining:** wire `-t browser` into the CLI/SDK selector (needs the async `start()` handshake); live ≥90-min idle measurement; Track B (cookie/SAPISIDHASH) now optional/secondary. |
