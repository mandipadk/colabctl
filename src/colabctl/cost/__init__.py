"""Cost engine: price discovery and cheapest-first routing inputs.

A backend-neutral price model (``GpuPrice``) behind a ``PriceSource`` chain with a
``PriceCatalog`` facade. The offline static table provides a deterministic floor, while live
feeds such as ComputePrices implement the same interface. The AWS Spot Advisor
